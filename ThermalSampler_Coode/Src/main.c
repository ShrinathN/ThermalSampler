...
int main(void)
{
  ...
  while (1)
  {
    ...
    dataPacket.device_address = (unique_id);
    dataPacket.temperature = (sensor_data.temperature != 0) ? (sensor_data.temperature) : (int8_t)temperature_c;
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, NRF_CS_Pin|NRF_CE_Pin|RFM95W_RESET_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : NRF_CS_Pin NRF_CE_Pin RFM95W_CS_Pin RFM95W_RESET_Pin
                           SENSOR_SW_Pin */
  GPIO_InitStruct.Pin = NRF_CS_Pin|NRF_CE_Pin|RFM95W_CS_Pin|RFM95W_RESET_Pin
                          |SENSOR_SW_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
